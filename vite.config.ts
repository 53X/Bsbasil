// This project was exported from Airo. Airo-only plugins are not part of this export.
import { defineConfig, type Plugin, type UserConfig, type ConfigEnv, type ViteDevServer } from "vite";
...
function ssrCjsCompatPlugin(): Plugin {
  const CJS_PACKAGES: string[] = ["react-router"];
  function rewriteNames(raw: string): string {
    return raw.split(",").map((s: string) => s.trim()).filter((s: string) => s && !s.startsWith("type ")).map((s: string) => s.replace(/^(\S+)\s+as\s+(\S+)$/, "$1: $2")).join(", ");
  }
  return {
    name: "ssr-cjs-compat",
    apply: "serve",
...
function worktreePreviewPlugin(): Plugin {
  const serverBundleCache = new Map<string, {
    app: unknown;
    mtimeMs: number;
  }>();
  return {
    ...
    configureServer(server: ViteDevServer) {
      server.middlewares.use(async (req, res, next) => {
        ...
        try {
          ...
          const app = cached.app as (req: IncomingMessage, res: ServerResponse, next: () => void) => void;
          app(req, res, () => {
...
function ssrDevPlugin(): Plugin {
  const worktreeBundleCache = new Map<string, {
    app: unknown;
    mtimeMs: number;
  }>();
  async function renderFromSource(server: ViteDevServer, routePath: string, req: IncomingMessage, res: ServerResponse): Promise<void> {
    try {
      ...
      const template: string = await readFile(path.resolve(__dirname, "index.html"), "utf-8");
      const html: string = template.replace("<!--app-head-->", result.head).replace("<!--app-html-->", result.html);
      res.statusCode = result.status;
      res.setHeader("Content-Type", "text/html");
      res.end(html);
    } catch (err) {
      ...
      res.statusCode = 500;
      res.end(JSON.stringify({
        error: `SSR render failed: ${message}`
      }));
    }
  }
  async function renderFromWorktreeBuild(runId: string, routePath: string, req: IncomingMessage, res: ServerResponse): Promise<void> {
    if (!/^[a-zA-Z0-9_-]+$/.test(runId)) {
      res.statusCode = 400;
      res.end(JSON.stringify({
        error: "Invalid worktree runId"
      }));
      return;
    }
...
export default defineConfig(async ({ mode, isSsrBuild }: ConfigEnv): Promise<UserConfig> => {
  const base: UserConfig = {
    ...
    }), ssrCjsCompatPlugin(), ssrDevPlugin(), worktreePreviewPlugin(), apiDevPlugin(), mediaAssetsPlugin(), formatOverridesPlugin(__dirname), contentPlugin()],
    resolve: {
    ...
    optimizeDeps: {
      include: ["react", "react-dom", "react-router", "motion/react", "html-to-image", "rrweb-snapshot"]
    },
    ssr: {
